...
# %%
df=pickle.load(open( os.path.join('07_models','all_results_df_val.p'),'rb'))
...
# %%
custom_dict = {'di_sc':0,'tetra_sc_tri_p':1}
...
for model_number in range(n_members):
    # load model
        print('loading ...' + os.path.join('07_models','tetra_sc_tri_p_'+part+"{:02d}".format(model_number)+'.h5'))
        model =  load_model( os.path.join('07_models','tetra_sc_tri_p_'+ part +"{:02d}".format(model_number)+'.h5') )
    # store in memory
        models.append(model)
...
zz=Counter(test_Y)
num_of_class=10
sample_w=[zz[i] for i in range(0,num_of_class,1)]
...
plt.clim(0,1)
plt.savefig('08_figures/tetra_sc_tri_p_CM_val_loss.png',bbox_inches="tight")
plt.show()
